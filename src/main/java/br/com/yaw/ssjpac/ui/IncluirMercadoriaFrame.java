...
public class IncluirMercadoriaFrame extends JFrame {
	...
	private Product loadMercadoriaFromPanel() {
		String nome = null;
		...
		try {
			if (!tfPreco.getText().trim().isEmpty()) {
				preco = Product.formatStringToPreco(tfPreco.getText());
			}
		...
		return new Product(id, nome, descricao, amount, preco, version);
	}
	...
	private void populaTextFields(Product m){
	...
	/**
	 * Limpa e carrega os campos da tela de acordo com objeto <code>Product</code>.
	 * @param m referência da <code>Product</code> que deve ser apresentada na tela.
	 */
	public void setMercadoria(Product m){
		resetForm();
		if (m != null) {
			populaTextFields(m);
	...
	/**
	 * @return uma nova instância de <code>Product</code> com os dados preenchidos do campos na tela.
	 */
	public Product getMercadoria() {
		return loadMercadoriaFromPanel();
	}
	
	/**
	 * @return o identificador da <code>Product</code> em edição. Retorna <code>null</code> em modo de inclusão.
	 */
